...
coffee-stir src/main.coffee | sed 's/write completed//' | coffee --compile --stdio | sed -e 's/insert_mode_here/%mode%/' -e 's/insert_version_here/%datetime%/' | tee generated_bundle.js | clip